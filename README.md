## Demonstration of search in redis values server-side with a special lua-script

This is a public page of the [GitHub Repository Redis-Search-Values-Demo](https://github.com/a-bobkov/redis-search-values-demo).
I proposed an approach to a maintenance task of searching in large amount of values stored in Redis.
A real use-case of applying the approach is described. A working example of code for NodeJS is provided.
...
So you have a question, what sessions are broken. It is not so easy to answer, because there are 37 millions of sessions, taking more than 20 gigabytes and all the sessions are stored in Redis 6. You know, that Redis is a very popular key-value storage, it is very fast and reliable, but provides poor functionality to investigate stored values. In Redis you can not simply query data with a filter by stored values.
...
The solution with SCAN parameter "COUNT 5000" has scanned 37 millions of sessions on a live remote server in 22 minutes and has found 604 broken sessions.
...
[<img src="coffee.png" width="250"/>](https://www.paypal.com/donate/?hosted_button_id=MYA8VXRL7Q4ZE)
